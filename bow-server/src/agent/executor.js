...
export class AgentExecutor {
    constructor(logger, executor, planner) {
    ...
    async execute(plan, context = {}) {
        ...
        for (const step of plan.steps) {
            ...
            const unsatisfiedDeps = step.dependencies.filter((dep) => !stepResultMap.has(dep) || !stepResultMap.get(dep).success);
    ...
    async executeStep(step, context) {
        ...
        for (let attempt = 0; attempt <= step.maxRetries; attempt++) {
            try {
                ...
                const result = await this.executeWithTimeout(this.executor.execute(step.toolName, step.input, fullContext), step.timeout);
                lastResult = result;
                if (result.success) {
                    return {
                        stepId: step.id,
