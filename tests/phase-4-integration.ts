...
import AIAgent from "@bow/server/agent/index.js";
import Planner from "@bow/server/agent/planner.js";
import AgentExecutor from "@bow/server/agent/executor.js";
...
async function runTests(): Promise<void> {
    ...
    try {
        ...
        logger.info(`Agent stats:`, stats as Record<string, unknown>);

        // Test 7: Tool Information
        logger.info("Test 7: Tool Information");
        const toolInfo = agent.getToolInfo();
        logger.info(`Tool info:`, toolInfo as Record<string, unknown>);
